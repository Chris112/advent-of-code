  ...
  const instructions = getInstructions(lines);
    ...
    //   console.log("Parsing instruction %o", instructionParts);
      ...
      // for (let i = 0; i < numberToMove; i++) {
        ...
        const boxes = stacks[fromStack - 1].splice(stacks[fromStack - 1].length - numberToMove, numberToMove);
        console.log("Moving box %o", boxes)
        // const boxes = stacks[fromStack - 1].splice(0, numberToMove);
        // console.log("Moving boxes %o", boxes)
        // if (!box) {
          // throw new Error("No box to move");
        // }
        stacks[toStack - 1].push(...boxes);
      // }
    }
